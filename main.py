...
cmd = "git clone https://github.com/Insane1708/Testcolab"
proc = subprocess.run(cmd, shell=True)
cmd = "apt update && apt install ffmpeg aria2"
proc = subprocess.run(cmd, shell=True)
cmd = "pip3 install -r /content/Testcolab/requirements.txt"
proc = subprocess.run(cmd, shell=True)
...
with open('/content/Testcolab/credentials.json', 'w') as file:
    file.write(json.dumps(credentials))

Working = False

if os.path.exists("/content/Testcolab/my_bot.session"):
    os.remove("/content/Testcolab/my_bot.session") # Remove previous bot session
    
print("\rStarting Bot....")

!cd /content/Testcolab/ && python3 -m colab_leecher
